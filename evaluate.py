...
def main(src_file, trg_file, pred_file):

    if export_filtered_pred:
        pred_output_file = open(os.path.join(filtered_pred_path, "predictions_filtered.txt"), "w")
    ...
    for data_idx, (src_l, trg_l, pred_l) in enumerate(zip(src_file, trg_file, pred_file)):
        num_src += 1
        # convert the str to token list
        pred_str_list = pred_l.strip().split(';')
        pred_str_list = pred_str_list[:num_preds]
        ...
        # print(trg_token_2dlist)
        # print(pred_token_2dlist)
        ...
        # print(stemmed_src_token_list)
        # print(stemmed_trg_token_2dlist)
        # print(stemmed_pred_token_2dlist)

        # Filter out duplicate, invalid, and extra one word predictions
        filtered_stemmed_pred_token_2dlist, num_duplicated_predictions = filter_prediction(disable_valid_filter, disable_extra_one_word_filter, stemmed_pred_token_2dlist)
        num_unique_predictions += (num_predictions - num_duplicated_predictions)

        # print(filtered_stemmed_pred_token_2dlist)
        ...
        # print(unique_stemmed_trg_token_2dlist)
        # separate present and absent keyphrases
        present_filtered_stemmed_pred_token_2dlist, absent_filtered_stemmed_pred_token_2dlist = separate_present_absent_by_source(stemmed_src_token_list, filtered_stemmed_pred_token_2dlist, match_by_str)

        # print(present_filtered_stemmed_pred_token_2dlist)
        # print(absent_filtered_stemmed_pred_token_2dlist)

        present_unique_stemmed_trg_token_2dlist, absent_unique_stemmed_trg_token_2dlist = separate_present_absent_by_source(
                stemmed_src_token_list, unique_stemmed_trg_token_2dlist, match_by_str)
        ...
        # unique_stemmed_trg_token_2dlist =  trg_token_2dlist
        # filtered_stemmed_pred_token_2dlist =  pred_token_2dlist
    ...
    result_txt_str_all, field_list_all, result_list_all,F_classification_all, F_ranking_all = report_stat_and_scores(num_filtered_predictions, num_unique_targets, num_src, score_dict, topk_dict['all'], 'all')
    result_txt_str_present, field_list_present, result_list_present,F_classification_present, F_ranking_present = report_stat_and_scores(num_present_filtered_predictions, num_present_unique_targets, num_src, score_dict, topk_dict['present'], 'present')
    result_txt_str_absent, field_list_absent, result_list_absent,F_classification_absent, F_ranking_absent = report_stat_and_scores(num_absent_filtered_predictions, num_absent_unique_targets, num_src, score_dict, topk_dict['absent'], 'absent')
    ...
    F_classification=[]
    F_ranking =[]
    # print(F_classification_all)
    F_classification.append(F_classification_all)
    F_classification.append(F_classification_present)
    F_classification.append(F_classification_absent)

    F_ranking.append(F_ranking_all)
    F_ranking.append(F_ranking_present)
    F_ranking.append(F_ranking_absent)

    # classification_file = open(os.path.join(exp_path, "classification.txt"), "w")
    # classification_file.write(str(F_classification))
    # classification_file.close()
    #
    # ranking_file = open(os.path.join(exp_path, "ranking.txt"), "w")
    # ranking_file.write(str(F_ranking))
    # ranking_file.close()

    # Write to files
    results_txt_file = open(os.path.join(exp_path, "results_log.txt"), "w")

    results_txt_file.write(result_txt_str)
    results_txt_file.close()

    results_tsv_file = open(os.path.join(exp_path, "results_log.tsv"), "w")
    ...
    print('\n\nThe full results for %s:' % pred_path)
    print(result_txt_str)
    print("Writing results into %s and takes %.2f" % (os.path.join(exp_path, "results_log.txt"), time.time() - t0))
    return
...
def report_stat_and_scores(num_filtered_predictions, num_unique_trgs, num_src, score_dict, topk_list, present_tag):
    ...
    classification_output_str, classification_field_list, classification_result_list,F_classification = report_classification_scores(
    ...
    ranking_output_str, ranking_field_list, ranking_result_list,F_ranking = report_ranking_scores(score_dict,
                                                                                        topk_list,
                                                                                        present_tag)
    result_txt_str += ranking_output_str
    field_list += ranking_field_list
    result_list += ranking_result_list
    return result_txt_str, field_list, result_list, F_classification, F_ranking
...
def report_classification_scores(score_dict, topk_list, present_tag):
    ...
    for topk in topk_list:
        ...
        F.append([macro_avg_precision_k, macro_avg_recall_k, macro_avg_f1_score_k])
    return output_str, field_list, result_list, F
...
def report_ranking_scores(score_dict, topk_list, present_tag):
    ...
    for topk in topk_list:
        ...
        F.append([map_k, avg_ndcg_k, avg_alpha_ndcg_k])
    return output_str, field_list, result_list, F
...
def readtxt(path):
    ...
    with open(path,'r',encoding='utf8') as f:
        ...
        print(len(lines))
        for i in lines:
            i=i.replace('\u3000','').replace('\n','').replace('\xa0','').replace('\ufeff','').replace(' ; ',';').replace('[UNK]','')
            data.append(i)
    return data
...
if __name__ == '__main__':
    ...
    for i in range(1,6):
        ...
        data = pd.read_csv('./data/kp20k/data.test.tsv', sep='\t', header=None)
        # print(data.iloc[:,1])
        src = list(data.iloc[:, 0])
        trg = list(data.iloc[:, 1])
        # pred_path = './test/kp20k/bt.txt'

        folder = '/Users/sdai/Desktop/lynn/pred/BT/kp20k/'+str(i)+'/'
        # src = readtxt(folder+'/all_src.txt')
        # trg = readtxt(folder+'/all_targets.txt')
        pred_path = folder+'/test-predictions.txt'
        pred = readtxt(pred_path)
        # pred_path='./test'
        exp_path = os.path.dirname(pred_path)
        assert os.path.exists(exp_path) is True
        filtered_pred_path = exp_path
        export_filtered_pred = False
        invalidate_unk = True
    ...
    # src=['']
    # trg=['multimode interference coupler;silicon on insulator;data compression;symmetric cosine transform;symmetric sine transform;image processing;karhunen loeve transform;all optical signal processing;beam propagation method']
    # pred = ['karhunen loeve transform;cosine transform;multimode interference;transfer matrix method;symmetric cosine transform;symmetric sine transform;beam propagation method;sine transform;multimode interference structures;silicon wire;bpm;unitary transforms;karhunen loeve;rate distortion criterion;optical symmetric cosine transform;cosine and sine transform;sine transforms;image compression;beam propagation;interference;multimode;symmetric sine transforms;interference structures;signal processing;computational efficiency;modified symmetric cosine transform;silicon wire waveguides;sst;wire waveguides;residual correlation transform;transfer;optical symmetric sine transform;transfer matrix;rate distortion;cosine and sine transforms;all optical;residual correlation;optical;rate distortion transform;beam waveguides;silicon;sct;beam;image processing;karhunen;optical symmetric;modified symmetric sine transform;optical filters;variety;wire']
    # print('target:',trg)
