...
model_dir = './test/stack_bah'
topic_path = './topic/test/topic_word_distribution.txt'
...
def main():
    ...
    def body(i,n,p_tensor, encoder_output,update_encoder_outputs):
        ...
        values = tf.expand_dims(encoder_output[i-1], 0)
        alpha = tf.zeros([200, 1], tf.float32)
        for j in range(50):  # topic_num=50
            query = tf.expand_dims(p_tensor[j], 0)
            attention_layer = BahdanauAttention(128)
            # score:[1, max_length, 1]
            score = attention_layer(query, values)
            # print(f"score: {score}")
            alpha = alpha + score[0]
        alpha = tf.truediv(alpha,50)
