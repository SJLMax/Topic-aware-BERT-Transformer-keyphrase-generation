...
def convert_single_example(ex_index, example, max_seq_length, max_tgt_length,tokenizer):
    """Converts a single `InputExample` into a single `InputFeatures`."""

    # noinspection PyProtectedMember
    sep_token_id = tokenizer._map_token_to_id('[SEP]')
    if len(example.src_text)>=512:
        example.src_text=example.src_text[:512]
    ...
    if example.tgt_text is not None:
        tgt_input_ids, _, _ = tokenizer.encode_text(text_a=example.tgt_text, max_seq_length=max_tgt_length)
...
def convert_examples_to_features_and_output_to_files(examples, max_seq_length, max_tgt_length,tokenizer, output_file):
    ...
    for (ex_index, example) in enumerate(examples):
        ...
        feature = convert_single_example(ex_index, example, max_seq_length, max_tgt_length,tokenizer)
...
def prepare_TFRecord_data(processor, tokenizer, data_dir, max_seq_length, max_tgt_length,output_dir):
    """
    Args:
    ...
    convert_examples_to_features_and_output_to_files(train_examples, max_seq_length, max_tgt_length,tokenizer, train_file)

    eval_examples = processor.get_dev_examples(data_dir)
    eval_file = os.path.join(output_dir, "eval.tf_record")
    convert_examples_to_features_and_output_to_files(eval_examples, max_seq_length, max_tgt_length,tokenizer, eval_file)

    test_examples = processor.get_test_examples(data_dir)
    test_file = os.path.join(output_dir, "test.tf_record")
    convert_examples_to_features_and_output_to_files(test_examples, max_seq_length, max_tgt_length,tokenizer, test_file)
